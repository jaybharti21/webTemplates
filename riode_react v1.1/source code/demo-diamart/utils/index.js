...
export const parallaxHandler = function () {
    ...
    if ( parallaxItems ) {
        for ( let i = 0; i < parallaxItems.length; i++ ) {
            ...
            let parallax = parallaxItems[ i ], yPos, parallaxSpeed = 1, offset = 50;

            if ( parallax.getAttribute( 'data-option' ) ) {
                parallaxSpeed = parseInt( parseOptions( parallax.getAttribute( 'data-option' ) ).speed );
                offset = parseInt( parseOptions( parallax.getAttribute( 'data-option' ) ).offset );
            }

            yPos = ( parallax.offsetTop - window.pageYOffset ) * 50 * parallaxSpeed / parallax.offsetTop + offset;
