...
export function scrollTopHandler( isCustom = true, speed = 15 ) {
    ...
    if ( isCustom && !isEdgeBrowser() ) {
        if ( document.querySelector( '.main .product-wrapper' ) ) {
            offsetTop = document.querySelector( '.main .product-wrapper' ).getBoundingClientRect().top + window.pageYOffset - document.querySelector( '.sticky-header' ).offsetHeight + 10;
        }
    } else {
        offsetTop = 0;
    }
