...
function PostTen(props) {
    const { post, isLazy = false, isOriginal = false, btnText = "Read more", btnAdClass = '', isButton = true } = props;

    return (
        <div className={`post overlay-zoom overlay-dark ${post.type === 'video' ? 'post-video' : ''}`}>
            {
            ...
            <div className="post-details">
                ...
                {
                    isButton ?
                        <ALink href={`/blog/single/${post.slug}`} className={`btn btn-link btn-underline ls-normal text-normal font-weight-normal ${btnAdClass}`}>{btnText}<i className="d-icon-arrow-right"></i></ALink>
                        : ''
                }
            </div>
        </div >
