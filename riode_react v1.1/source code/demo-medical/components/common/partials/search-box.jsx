...
function SearchForm () {
    ...
    return (
        <div className="header-search hs-toggle d-block">
            <a href="#" className="search-toggle d-flex align-items-center" role="button" onClick={ onSearchClick }><i className="d-icon-search"></i></a>
            <form action="#" method="get" onSubmit={ onSubmitSearchForm } className="input-wrapper mt-4">
                ...
                <div className="live-search-list bg-white">
                    { search.length > 2 && data && data.products.data.map( ( product, index ) => (
                        <ALink href={ `/product/default/${ product.slug }` } className="autocomplete-suggestion" key={ `search-result-${ index }` }>
                            ...
                            <div className="search-name ml-2" dangerouslySetInnerHTML={ removeXSSAttacks( matchEmphasize( product.name ) ) }></div>
