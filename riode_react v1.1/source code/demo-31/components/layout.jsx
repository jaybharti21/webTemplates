...
function Layout({ children, closeQuickview }) {
    ...
    let urlListener = router.pathname.substr(-5, 5) === '/shop' || router.pathname.substr(-5, 5).includes('grid');
    ...
    return (
        <>
            <div className={`page-wrapper ${router.pathname.includes('off-canvas') || urlListener ? 'off-canvas' : ''}`}>
                <Header />
