...
export default function ToolBox ( props ) {
    const { type = "left", totalCount = 0 } = props;
    ...
    return (
        <nav className={ `toolbox sticky-toolbox sticky-content fix-top ${ type === "horizontal" ? 'toolbox-horizontal' : '' }` }>
            ...
            <div className="toolbox-left">
                ...
                {
                    type === "navigation" && totalCount > 0 ?
                        <p className="toolbox-item show-info mr-sm-auto">Showing <span>{ perPage * ( page - 1 ) + 1 } - { Math.min( perPage * page, totalCount ) } of { totalCount }</span>Products</p>
                        : ''
                }
            </div>
