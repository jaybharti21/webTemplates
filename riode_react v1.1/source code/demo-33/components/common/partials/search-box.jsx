...
function SearchForm({ homesearch }) {
    ...
    useEffect(() => {
        document.querySelector('.search-box-wrapper.show-results') && document.querySelector('.search-box-wrapper.show-results').classList.remove('show-results');
    }, [router.pathname])
    ...
    function showSearchBox(e) {
        e.preventDefault();
        e.stopPropagation();
        e.currentTarget.closest('.search-box-wrapper').classList.toggle('show');
    }

    function onBodyClick(e) {
        if (e.target.closest('.search-box-wrapper')) return e.target.closest('.search-box-wrapper').classList.contains('show-results') || e.target.closest('.search-box-wrapper').classList.add('show-results');

        document.querySelector('.search-box-wrapper.show') && document.querySelector('.search-box-wrapper.show').classList.remove('show');
        document.querySelector('.search-box-wrapper.show-results') && document.querySelector('.search-box-wrapper.show-results').classList.remove('show-results');
    }
    ...
    return (
        <div className={`search-box-wrapper ${homesearch ? '' : 'header-search hs-simple'}`}>
            <form action="#" method="get" onSubmit={onSubmitSearchForm} className={`input-wrapper ${homesearch ? 'mx-auto' : ''}`}>
